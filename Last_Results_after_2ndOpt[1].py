...
def calc_last_yh(th_df, ds, ro, y_column_name, best_vals):
    ...
    nd_idx = best_vals[1]
    rd_idx = best_vals[2]
    Syh = y_h_rslt[st_idx] + y_h_rslt[nd_idx] + y_h_rslt[rd_idx]
    Myh = round((Syh / 3),4)
    return Myh

def select_final_mating(vals_3_idx):
    vals_idx = []
    for i in range(3):
        max_qual_idx = np.argmax(vals_3_idx)
        vals_idx.append(max_qual_idx)
        vals_3_idx[max_qual_idx] = -1
    return vals_idx
...
for i in range(len(dataSe)):
    ...
    yh_af = get_best_yh(thet_df, dataSe, i, 'Total_Bilirubin')
    yh_af_val = yh_af[0]
    yh_af_idx = yh_af[1]
...
for i in range(len(dataSe)):
    y_val = dataSe.iat[i,2]
    yh_bef = calc_yh(mi_thet_df, dataSe, i, 'Total_Bilirubin')
    yh_af1 = calc_yh(thet_df, dataSe, i, 'Total_Bilirubin')
    yh_af = calc_last_yh(thet_df, dataSe, i, 'Total_Bilirubin', best_vals)
    abs_bef = np.abs(y_val - yh_bef)
    abs_af = np.abs(y_val - yh_af)
    abs_af1 = np.abs(y_val - yh_af1)
    final_result.loc[i] = [y_val, yh_bef, yh_af1, yh_af, abs_bef, abs_af1, abs_af]
    
final_result.to_csv("best_yh_af1_af2.csv", index=False)
print('Done')
